...
def algorithm(name, inp=None, out=None, doc=None, label=None):
    al = Algorithm()
    al.caption(name)
    alc = al.algorithmic(inp, out)
    if doc is not None:
        if label is not None:
            doc.pre_append(Command("floatname", arguments=("algorithm", label[0])))
            doc.define([r"\algorithmicrequire", r"\algorithmicensure"],
